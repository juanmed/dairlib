...
#include "parameters/box_repositioning_c3_controller_options.h"
#include "parameters/box_repositioning_config.h"
#include "parameters/box_repositioning_target_config.h"
...
using examples::plate_balancing::C3SceneConfig;
using examples::plate_balancing::LcmChannelConfig;
using examples::plate_balancing::systems::FrankaKinematics;
...
C3Controller* AddC3ControllerToBuilder(
    ...
    BoxRepositioningC3ControllerOptions& controller_options,
...
int DoMain(std::string plate_balancing_config, bool is_simulation) {
  ...
  BoxRepositioningConfig main_config =
      drake::yaml::LoadYamlFile<BoxRepositioningConfig>(plate_balancing_config);
  BoxRepositioningTargetConfig target_config =
      drake::yaml::LoadYamlFile<BoxRepositioningTargetConfig>(
          main_config.box_repositioning_target_config_file);
  ...
  plant_franka.WeldFrames(
      ...
      plant_franka.GetFrameByName("gripper", end_effector_index), T_EE_W);

  plant_franka.Finalize();
  auto franka_context = plant_franka.CreateDefaultContext();

  // Create MultibodyPlant for the box
  MultibodyPlant<double> plant_box(0.0);
  Parser parser_box(&plant_box, nullptr);
  parser_box.AddModels(scene_params.object_models[0]);
  plant_box.Finalize();
  auto box_context = plant_box.CreateDefaultContext();
  ...
  for (size_t i = 0; i < scene_params.object_models.size(); ++i) {
  ...
  std::vector<drake::geometry::GeometryId> end_effector_contact_points =
      plant_for_lcs.GetCollisionGeometriesForBody(
          plant_for_lcs.GetBodyByName("gripper"));
  for (size_t i = 0; i < environment_model_indices.size(); ++i) {
  ...
  std::vector<drake::geometry::GeometryId> box_geoms =
      plant_for_lcs.GetCollisionGeometriesForBody(
          plant_for_lcs.GetBodyByName("box"));
  std::unordered_map<std::string, std::vector<drake::geometry::GeometryId>>
      contact_geoms;
  contact_geoms["GRIPPER"] = end_effector_contact_points;
  contact_geoms["BOX"] = box_geoms;

  // Define contact pairs for the LCSFactorySystem
  std::vector<SortedPair<GeometryId>> contact_pairs;
  for (auto geom_id : contact_geoms["GRIPPER"]) {
    contact_pairs.emplace_back(geom_id, contact_geoms["BOX"][0]);
  }
  ...
  auto box_state_receiver = builder.AddSystem<ObjectStateConsumer>(plant_box);
  auto reduced_order_model_receiver = builder.AddSystem<FrankaKinematics>(
      plant_franka, franka_context.get(), plant_box, box_context.get(),
      scene_params.end_effector_name, "box",
  ...
  auto box_repositioning_target =
      builder.AddSystem<BoxRepositioningTargetGenerator>(
          plant_box, scene_params.end_effector_thickness,
          target_config.near_target_threshold);
  box_repositioning_target->SetRemoteControlParameters(
  ...
  std::vector<int> input_sizes = {4, 5, 4, 5};
  ...
  auto end_effector_zero_velocity_source =
      builder.AddSystem<drake::systems::ConstantVectorSource>(
          VectorXd::Zero(5));
  builder.Connect(
      box_repositioning_target->get_output_port_end_effector_target(),
      target_state_mux->get_input_port(1));
  builder.Connect(box_repositioning_target->get_output_port_box_target(),
  ...
  builder.Connect(
      box_repositioning_target->get_output_port_box_velocity_target(),
  ...
  builder.Connect(box_state_sub->get_output_port(),
                  box_state_receiver->get_input_port());
  builder.Connect(box_state_receiver->get_output_port(),
                  reduced_order_model_receiver->get_input_port_object_state());
  builder.Connect(box_state_receiver->get_output_port(),
                  box_repositioning_target->get_input_port_box_state());
  ...
  builder.Connect(radio_to_vector->get_output_port(),
                  box_repositioning_target->get_input_port_radio());
  ...
  const std::string path =
      "/home/juanmedrano_eng/repos/dairlib/examples/box-repositioning/diag.dot";
  std::ofstream graphviz(path);
  std::map<std::string, std::string> options_gv{{"plant/split", "I/O"}};
  graphviz << shared_diagram->GetGraphvizString({}, options_gv);
  ...
  LcmHandleSubscriptionsUntil(
      &lcm, [&]() { return box_state_sub->GetInternalMessageCount() > 1; });
  loop.Simulate();
...
int main(int argc, char* argv[]) {
  ...
  return dairlib::examples::box_repositioning::DoMain(
      FLAGS_box_repositioning_config, FLAGS_simulation);
}