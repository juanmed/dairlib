 ...
 */
class BoxRepositioningTargetGenerator
 ...
 public:
   ...
   */
  BoxRepositioningTargetGenerator(
  ...
  const drake::systems::InputPort<double>& get_input_port_box_state() const {
    return this->get_input_port(box_state_port_);
  }
  const drake::systems::OutputPort<double>&
  get_output_port_end_effector_target() const {
    return this->get_output_port(end_effector_target_port_);
  }
  const drake::systems::OutputPort<double>& get_output_port_box_target()
      const {
    return this->get_output_port(box_target_port_);
  }
  const drake::systems::OutputPort<double>&
  get_output_port_box_velocity_target() const {
    return this->get_output_port(box_velocity_target_port_);
  }
   ...
   */
  void SetRemoteControlParameters(const Eigen::Vector4d& first_target,
                                  const Eigen::Vector4d& second_target,
                                  const Eigen::Vector4d& third_target,
 ...
 private:
  ...
  void CalcBoxTarget(const drake::systems::Context<double>& context,
                      drake::systems::BasicVector<double>* target) const;
  /// Calculates the tray velocity target output.
  void CalcBoxVelocityTarget(
  ...
  drake::systems::InputPortIndex
      box_state_port_;  ///< Input port for box state.
  drake::systems::OutputPortIndex
      end_effector_target_port_;  ///< Output port for end effector target.
  drake::systems::OutputPortIndex
      box_target_port_;  ///< Output port for box target.
  drake::systems::OutputPortIndex
      box_velocity_target_port_;  ///< Output port for box velocity target.
  ...
  double end_effector_thickness_;    ///< End effector t.
  Eigen::Vector4d first_target_;     ///< First target position.
  Eigen::Vector4d second_target_;    ///< Second target position.
  Eigen::Vector4d third_target_;     ///< Third target position.
  Eigen::VectorXd target_box_state_; ///< Current target
